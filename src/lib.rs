...
pub mod scrape;
use crate::scrape::StartUrl;

use futures::stream::{self, Iter as StreamIter, StreamExt, TryStreamExt};
...
impl QuickCrawlerBuilder {
    ...
    pub fn with_start_urls<'a>(
        &'a mut self,
        start_urls: Vec<StartUrl>,
    ) -> &'a mut QuickCrawlerBuilder {
    ...
    pub fn finish(&self) -> Result<QuickCrawler, QuickCrawlerError> {
        let data = self
            .start_urls
            .as_ref()
            .ok_or(QuickCrawlerError::NoStartUrls)?;
...
impl Stream for DataDistributor {
    ...
    fn poll_next<'a>(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let Self { receiver } = &mut *self;
        ...
        if empty {
            ...
            let mut unwrapped_fut = Box::pin(async move { receiver.recv().await.unwrap() });
...
impl<'a> QuickCrawler<'a> {
    pub async fn process(self) -> Result<Vec<DataFromScraperValue>, QuickCrawlerError> {
        ...
        let (data_to_manager_sender, data_to_manager_receiver) =
            channel::bounded::<DataFromScraperValue>(100);

        let limiter = self.limiter;
        let request_handler = self.request_handler;

        let stream_senders_fut: Pin<Box<dyn Future<Output = Result<(), QuickCrawlerError>>>> =
            Box::pin(
                self.start_urls
                    .map(|url| {
                        (
                            data_to_manager_sender.clone(),
                            url,
                            limiter.clone(),
        ...
        let data_distributor_stream_fut: Pin<Box<dyn Future<Output = Vec<DataFromScraperValue>>>> =
            Box::pin(data_distributor_stream.collect());
        ...
        let stream_complete_fut = async move {
            ...
            let _res = data_to_manager_sender2
                .send(DataFromScraperValue::Complete)
                .await;
...
mod tests {
    ...
    fn with_start_urls_finished() -> () {
        ...
        let _url4 = format!("{}{}", base_url, path4);
        ...
        let _m2 = mock(
            "GET",
            Matcher::Regex(r"^/compilation/another-meal-1$".to_string()),
        )
        .with_body(format!(
            r#"
        ...
        let _m2 = mock(
            "GET",
            Matcher::Regex(r"^/compilation/another-meal-(2|3)$".to_string()),
        )
        .with_body(format!(
            r#"
                    <div class='ingredients-prep'>
        ...
        let _m3 = mock(
            "GET",
            Matcher::Regex(r"^/compilation/other-meal-1$".to_string()),
        )
        .with_body(format!(
            r#"
                    <div class='ingredients-prep'>
        ...
        builder
            .with_start_urls(start_urls)
            .with_limiter(limiter)
            .with_request_handler(|config: RequestHandlerConfig| async move {
                // ... use any request library, like reqwest
                surf::get(config.url.clone())
                    .recv_string()
                    .await
        ...
        let crawler = builder
            .finish()
            .map_err(|_| "Builder could not finish")
            .expect("no error");
        let res = task::block_on(async { crawler.process().await });
